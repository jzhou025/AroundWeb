...
const (
	...
	POST_TYPE  = "post"
	DISTANCE   = "200km"
	// ES_URL     = "http://35.236.73.166:9200"
	ES_URL      = "http://localhost:9200"
	BUCKET_NAME = "post-image-25"
	API_PREFIX  = "/api/v1"
)
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	/*
		...
		if err != nil {
			http.Error(w, "Failed to save post to BigTableh", http.StatusInternalServerError)
...
func saveToBigTable(p *Post, id string) error {
	...
	bt_client, err := bigtable.NewClient(ctx, "around25", "around-post")
	if err != nil {
		return err
	}
...
func saveToGCS(r io.Reader, bucketName, objectName string) (*storage.ObjectAttrs, error) {
	...
	// ACL is the access contrl, in this case, all user could access the storage
	if err = object.ACL().Set(ctx, storage.AllUsers, storage.RoleReader); err != nil {
		return nil, err
	}

	// attrs include the url of the image
